...
class Editor(Fstream):
    ...
    def opt_Exp(self):
        if not data.table_empty:
            ...
            self.dial(f'''当前数据库中无表格,请先创建表格！''',catagory="Warning")
    ...
    def opt_New(self):
        name = "表格"+str(len(data.table_list)+1)
        while name in data.table_list:
            name = name+"_2"
        ...
        self.input_new_edit.setText(name)
    ...
    def data_Add(self):
        ...
        try:
            ...
            self.dial(f'''请按格式输入数据！''',catagory="Error")
            return
